...
let extension = {
    ...
    getEnergy: function (useTerminal, useStorage, useDropped, id) {
        ...
        if (arguments.length === 0) {
            let source = this.pos.findClosestByRange(FIND_STRUCTURES, {
        ...
        if (id) {
            ...
            if (source) {
                ...
                if (source.length > 1) {
                    let target = this.pos.findClosestByRange(source[0], {
    ...
    recycle: function () {
        ...
        if (suicidePos) {
            if (this.pos.getRangeTo(suicidePos) > 0) {
                ...
                const spawn = this.pos.findInRange(this.room.structures[STRUCTURE_SPAWN],1)
                if (spawn) {
